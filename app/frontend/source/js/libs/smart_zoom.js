  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
      ...
      zoom : function(scaleToAdd, globalRequestedPosition, duration){
        let smartData, globalRequestedX, globalRequestedY, containerRect;

        smartData = targetElement.data('smartZoomData');
        containerRect = getRect(smartData.containerDiv);

        if(!globalRequestedPosition){ // use target center if globalRequestedPosition is not set
          globalRequestedX = containerRect.x + containerRect.width/2;
          globalRequestedY = containerRect.y + containerRect.height/2;
        }else{
          globalRequestedX = globalRequestedPosition.x + containerRect.width/2;
          globalRequestedY = globalRequestedPosition.y + containerRect.height/2;
        }
        ...
        var positionGlobalXDiff = globalRequestedX - targetRect.x; // the position difference between with the target position
        var positionGlobalYDiff = globalRequestedY - targetRect.y;
